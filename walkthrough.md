...
- **Cơ chế:** Đảm bảo tính nhất quán giữa Booking -> Scheduling (Slot) -> Meeting.
- **Quy trình:**
    1. **Booking Service:** Tạo đặt chỗ ở trạng thái `INITIALIZED`, bắn `pitch_booking_initiated`.
    2. **Scheduling Service:** Giữ chỗ (`BOOKED`), bắn `slot_confirmed`.
    3. **Meeting Service:** Tạo phòng họp Zoom, bắn `meeting_auto_created`.
    4. **Booking Service:** Chuyển trạng thái sang `CONFIRMED`.
- **Bù trừ (Compensation):** Nếu tạo Meeting thất bại, **Scheduling Service** sẽ tự động giải phóng Slot về `AVAILABLE`.
