...
### [Component] API Gateway Advanced Plugins (Infrastructure)

#### [MODIFY] [setup_apisix_routes.py](file:///d:/Django_project/api-gateway_vs_bff/setup_apisix_routes.py)
- **Rate Limiting (`limit-req`)**: Áp dụng giới hạn 10 request/giây, burst 5 cho toàn bộ các route để chống tấn công Brute-force/DoS.
- **Logging (`file-logger`)**: Ghi log chi tiết các request vào file `/usr/local/apisix/logs/access.log` để phục vụ audit và debug.
- **Authentication (`jwt-auth`)**: Kích hoạt plugin JWT cho các route `/web/*` và `/admin/*`. 
  - *Lưu ý*: Sẽ cần tạo Consumer trong APISIX để khớp với JWT token từ Django.
- **Monitoring (`prometheus`)**: Đảm bảo plugin này luôn được bật trên các route quan trọng.
